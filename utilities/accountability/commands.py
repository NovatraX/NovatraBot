...
class AccountabilityCommands:
    ...
    async def _update_accountability_channel(self, ctx, user_id, today):
        ...
        tasks_embed = discord.Embed(
            ...
            or "No tasks logged yet today. Use `/log add [task]` to log your first task!",
            color=0xAAB99A,
        )
    ...
    async def add_command(self, ctx: discord.ApplicationContext, task: str):
            ...
            if user_stats:
                ...
                if last_logged != today:
                    ...
                    self.db.update_user_stats(
                        user_id, novacoins, streak, today, highest_streak
                    )
            else:
        ...
        response_msg.add_field(
            ...
            value=f"{streak} Days"
            + (f" (Record: {highest_streak})" if highest_streak > streak else ""),
            inline=True,
        )
        response_msg.add_field(
    ...
    async def delete_command(self, ctx: discord.ApplicationContext, task_number: int):
        ...
        if not task_info:
            await ctx.respond(
                "❌ Invalid task number or you have no logged tasks for today!"
            )
            return
    ...
    async def stats_command(
        ...
        user_stats = self.db.get_user_stats(user_id)
        if user_stats:
            novacoins, streak, _, highest_streak, total_tasks, weekly_target = (
                user_stats
            )
            weekly_count = self.db.get_weekly_tasks_count(user_id)
            weekly_progress = (
                min(weekly_count / weekly_target * 100, 100) if weekly_target > 0 else 0
            )
        else:
        ...
        if weekly_logs:
            ...
            for day, tasks in sorted(tasks_by_day.items()):
                ...
                for task, reward in tasks:
                    summary_text += (
                        f"• {task}"
                        + (f" (+{reward} coins)" if reward > 0 else "")
                        + "\n"
                    )
                summary_text += "\n"
    ...
    async def set_weekly_target_command(
        self, ctx: discord.ApplicationContext, target: int
    ):
        ...
        if target <= 0 or target > 100:
            await ctx.respond(
                "❌ Weekly target must be between 1 and 100 tasks.", ephemeral=True
            )
            return
    ...
    async def store_command(self, ctx: discord.ApplicationContext):
        ...
        if not store_items:
            await ctx.respond(
                "🏪 The store is currently empty. Check back later!", ephemeral=True
            )
            return
    ...
    async def buy_item_command(self, ctx: discord.ApplicationContext, item_id: int):
        ...
        self.db.cursor.execute(
            "SELECT name, description, price FROM store_items WHERE id = ? AND is_active = 1",
            (item_id,),
        )
        ...
        if not item:
            await ctx.respond(
                "❌ Invalid item ID or the item is no longer available.", ephemeral=True
            )
            return
        ...
        if not user_stats or user_stats[0] < price:
            await ctx.respond(
                f"❌ You don't have enough NovaCoins to buy this item. You need {price} coins.",
                ephemeral=True,
            )
            return
    ...
    async def inventory_command(self, ctx: discord.ApplicationContext):
        ...
        if not user_items:
            await ctx.respond(
                "📦 Your inventory is empty. Buy items from the store with `/log store`!",
                ephemeral=True,
            )
            return
        ...
        if used_items:
            used_text = "\n".join(
                [f"**{i + 1}.** {item[1]} - Used" for i, item in enumerate(used_items)]
            )
            inventory_embed.add_field(
                name="✅ Used Items",
                value=used_text,
    ...
    async def use_item_command(self, ctx: discord.ApplicationContext, item_number: int):
        ...
        if not unused_items:
            await ctx.respond(
                "❌ You don't have any unused items in your inventory.", ephemeral=True
            )
            return

        if item_number < 1 or item_number > len(unused_items):
            await ctx.respond(
                "❌ Invalid item number. Check your inventory with `/log inventory`.",
                ephemeral=True,
            )
            return
    ...
    async def add_item_command(
        self, ctx: discord.ApplicationContext, name: str, price: int, description: str
    ):
        ...
        if price < 1:
            await ctx.respond(
                "❌ Item price must be at least 1 NovaCoin.", ephemeral=True
            )
            return
        ...
        if success:
            ...
            await ctx.respond(
                "❌ An item with that name already exists in the store.", ephemeral=True
            )
    ...
    async def set_reminder_command(self, ctx: discord.ApplicationContext, time: str):
        ...
        if not re.match(r"^([01]\d|2[0-3]):([0-5]\d)$", time):
            await ctx.respond(
                "❌ Please enter a valid time in 24-hour format (HH:MM), e.g., 09:00 or 18:30.",
                ephemeral=True,
            )
            return
    ...
    async def delete_reminder_command(self, ctx: discord.ApplicationContext):
        ...
        if not reminder_time:
            await ctx.respond(
                "⚠️ You don't have any active reminders to delete.", ephemeral=True
            )
            return
    ...
    async def check_reminder_command(self, ctx: discord.ApplicationContext):
        ...
        if not reminder_time:
            await ctx.respond(
                "ℹ️ You don't have any active reminders set. Use `/log reminder set` to create one!",
                ephemeral=True,
            )
            return
